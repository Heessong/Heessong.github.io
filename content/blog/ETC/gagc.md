...
![갸츠비](https://user-images.githubusercontent.com/58321429/121802874-b255ab00-cc79-11eb-9faa-5dbd0670be3d.gif)

이렇게 마크다운 에디터를 사용해 다양한 카테고리의 글들을 트리구조로 쉽게 관리 할 수 있으며 글을 쓰는 즉시 마크다운 문법이 적용되어 실시간으로 확인하며 글을 쓸수 있고 글을 작성완료 한 후 명령어 하나만 실행하면 바로 블로그가 업데이트 된다! 👍👍👍👍👍

그리고 다른 블로그서비스들은 기본적으로 검색결과에 잘뜨지만 깃허브블로그는 따로 설정을 해주지않으면 구글이나 네이버의 검색결과에 보여지지 않아서 정말 나만의 일기장처럼 되어버린다. 

편안한 길 냅두고 멀리돌아가서 고생하는 나도 참..😋 그래도 누가 시켜서가 아니라 순전히 나의 호기심때문에 고생하는건 밤새서해도 재밌는거같다 아무리 귀찮아도 깃허브 블로그로 해야지..😗 

그래서 깃허브 블로그를 네이버나 구글검색결과에 노출시키는 방법을 찾아봤었는데  `구글 서치 콘솔`을 적용하면 검색결과에 뜨게 할 수 있었다. 

그때가 지금으로부터 3주전쯤이였나..? 

그때 대충대충 적용하고 자꾸 오류가나서 에이 안되나보다하고 잊고있었는데 갑자기 메일 한통이 왔다
...
이어서 네이버 검색엔진도 설정해봤는데 구글보다 훨씬 쉽게 잘되어있어서 바로 적용했다 😋😋😋